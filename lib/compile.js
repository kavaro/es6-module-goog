...
module.exports = function (file, code, options) {
    ...
    var modImports = googImports.map(function (variable) {
        var prop = variable.prop ? '.' + variable.prop : '';
        return 'var ' + variable.name + ' = ' + options.namespacePre + variable.namespace.join(options.namespaceSep) + prop + ';'
    }).join('');
    var modExports = googExports.map(function (variable) {
        var prop = variable.prop ? '.' + variable.prop : '';
        return options.namespacePre + variable.namespace.join(options.namespaceSep) + prop + ' = ' + variable.name + ';'
    }).join('\n');
    return {
        deps: deps,
        code: [
